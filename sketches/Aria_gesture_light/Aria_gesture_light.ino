...
const long colorWipeInterval = 50;
const long turnOffInterval = 1000;
...
void loop()
{
  ...
  //Serial.println(Wheel(map(sensorValue,0,1023,0,255)));
  col = Wheel(sensorValue);

  // UPDATE COLOUR
  if(previousColour!=col && aP!=0 && dir!=1 && dir!=2 && dir!=4)
  {
  ...
  if(ret>0) {
    ...
    if (ret&0x08) {
      Serial.print("Right,  ");
      aP = (dir==2)? strip.numPixels()-1:aP;
      dir = (dir==3)? dir:4;
      Serial.println(aP);
      
// colorWipe(strip.Color(255, 0, 0), 50, -1);
    }
  }
if(dir == 2)
{
...
if(dir == 3)
{
 ...
 colorWipe(col, colorWipeInterval, 1);
  
}
if(dir == 4)
{

 colorWipe(strip.Color(0, 0, 0), colorWipeInterval, -1);
...
void colorWipe(uint32_t c, uint8_t wait, int pointerDir) {
...
if (currentMillis - previousMillis >= wait) {
 previousMillis = currentMillis;
  Serial.println(aP);
   ...
   if(aP>=0 && aP<strip.numPixels())
   {
    Serial.println("working lights");
    Serial.println(aP);
   strip.setPixelColor(aP, c);
   //strip.setBrightness(BRIGHTNESS);
   strip.show();
   ...
   {
    dir = 0;
    aP=(pointerDir==1)? strip.numPixels()-1:0;
    Serial.println("stop");
   }
...
uint32_t Wheel(int WheelPos) {
  ...
  if(WheelPos >= 256 && WheelPos <= 511) {
    WheelPos -= 256;
    return strip.Color(0,255-WheelPos, WheelPos);
  }
  if(WheelPos >= 512 && WheelPos <= 767) {
    WheelPos -= 512;
    return strip.Color(0,WheelPos, 255);
  }
  if(WheelPos >= 768 && WheelPos <= 1023) {
    WheelPos -= 768;
    return strip.Color(WheelPos,255, 255);
  }
  else{
...
if (currentMillis - previousMillis >= wait) {
 previousMillis = currentMillis;
    ...
    for(i=0; i<strip.numPixels(); i++) {
      strip.setPixelColor(i, simpleWheel((i+j) & 255));
    }
    strip.show();
  }
  else
...
void rainbowCycle(uint8_t wait) {
  ...
  for(j=0; j<256*5; j++) { // 5 cycles of all colors on wheel
    for(i=0; i< strip.numPixels(); i++) {
      strip.setPixelColor(i, simpleWheel(((i * 256 / strip.numPixels()) + j) & 255));
    }
    strip.show();
    delay(wait);
  }
...
void theaterChaseRainbow(uint8_t wait) {
  for (int j=0; j < 256; j++) {     // cycle all 256 colors in the wheel
    for (int q=0; q < 3; q++) {
      for (uint16_t i=0; i < strip.numPixels(); i=i+3) {
        strip.setPixelColor(i+q, simpleWheel( (i+j) % 255));    //turn every third pixel on
      }
      strip.show();
...
uint32_t simpleWheel(byte WheelPos) {
